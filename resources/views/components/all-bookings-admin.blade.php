...
<div class="bg-[#1f2937] rounded-2xl p-5 shadow-md border border-gray-700 text-white flex flex-col justify-between w-full max-w-[320px] mx-auto">
    <div>
        <h2 class="text-lg font-semibold text-indigo-300 mb-3">👤 Użytkownik: <span class="text-white">{{ $user }}</span></h2>

        <ul class="text-sm space-y-1">
            <li>🎬 <strong>Tytuł:</strong> {{ $tytul }}</li>
            <li>⏱ <strong>Czas trwania:</strong> {{ $czas_trwania }} min</li>
            <li>📅 <strong>Data seansu:</strong> {{ $data }}</li>
            <li>🎟 <strong>Miejsca:</strong> {{ $liczba_miejsc }}</li>
            <li>
    ...
    <div class="flex justify-center gap-2 mt-5 flex-wrap">
        <a href="{{ route('admin.rezerwacje.editView', $id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded-md text-sm font-medium flex items-center justify-center gap-1">
            ✏️ Edytuj
        </a>
