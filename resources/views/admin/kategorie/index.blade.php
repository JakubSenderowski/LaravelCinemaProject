<x-default>
    <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-8 space-y-4 md:space-y-0">
        <form action="{{ route('admin.kategorie.search') }}" method="GET" class="flex-1">
            <input
                name="q"
                type="text"
                value="{{ request('q') }}"
        ...
        <h1 class="text-2xl font-bold text-brown text-center w-full md:w-auto">Lista kategorii</h1>

        <div class="flex gap-2">
            <a
                href="{{ route('admin.kategorie.create') }}"
            ...
            <a
                href="{{ route('admin.kategorie.index') }}"
                class="px-3 py-1.5 text-sm font-medium rounded-md bg-primary text-white hover:bg-accent transition-colors flex items-center justify-center gap-1"
            >
                ➕ Powrót
            </a>
